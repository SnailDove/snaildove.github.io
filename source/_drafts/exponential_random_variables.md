...
$$
![](./Figure_3.5_of_The_PDF_of_an_exponential_random_variable.png)

指数型随机变量能够对相关事件发生的时间建模，例如：消息到达计算机的时间，设备的使用寿命，灯泡的寿命，事故发生的时间等等。将在后面的章节看到指数型随机变量与几何随机变量紧密关联，几何随机变量也与相关事件发生的（离散）时间相关联。指数型随机变量将在第六章随机过程的学习中扮演重要的角色。但是目前为止，仅仅视它为一种特殊的可分析追中的随机变量。
